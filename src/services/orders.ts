...
export const OrderService = {
    async createOrder(input: CreateOrderInput): Promise<Order> {
        ...
        const orderPayload = {
            ...
            // order_number is auto-generated by DB sequence (nextval)
            source: 'web'
        };
        ...
        for (const item of input.items) {
            if (!item.variant_id) continue; // Skip simple products without variants
            ...
            if (stockError) {
                ...
                // Rollback: delete the order and its items
                await supabase.from('order_items').delete().eq('order_id', createdOrder.id);
                await supabase.from('orders').delete().eq('id', createdOrder.id);
                throw new AppError('One or more items are out of stock. Please try again.', 'INSUFFICIENT_STOCK', 400);
            }
        }
